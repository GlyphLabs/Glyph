...
class Utils(Cog):
    ...
    @option(
        name="member",
        description="The member you want get information",
        type=Member,
        required=False,
    )
    async def userinfo(self, ctx: ApplicationContext, member: Member):
    ...
    async def _help(self, ctx: ApplicationContext):
        ...
        if (
            ...
            and not await self.bot.db.get_guild_settings(
                ctx.guild.id, auto_insert=False
            )
        ):  # type: ignore
    ...
    @option(
        name="msgcontent",
        description="What the webhook should say",
        type=str,
        required=True,
    )
    ...
    async def avatar(self, ctx: ApplicationContext, member: str):
        if not member:
